...
def generateUserProfiles(R, d, seed, sparkContext, N):
    ...
    numUsers = U.count()
    randRDD = RandomRDDs.normalVectorRDD(
        sparkContext, numUsers, d, numPartitions=N, seed=seed)
...
def generateItemProfiles(R, d, seed, sparkContext, N):
    ...
    numItems = V.count()
    randRDD = RandomRDDs.normalVectorRDD(
        sparkContext, numItems, d, numPartitions=N, seed=seed)
...
def joinAndPredictAll(R, U, V, N):
    ...
    """
    RV = R.map(lambda (i, j, rij): (j, (i, rij)))\
        .join(V, numPartitions = N)\
        .map(lambda (j, ((i, rij), vj)): (i, (j, rij, vj)))
    return RV.join(U, numPartitions = N)\
        .map(lambda (i, ((j, rij, vj), ui)): (i, j, pred_diff(rij, ui, vj), ui, vj))\
        .repartition(N)
...
def normSqRDD(profileRDD, param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        λ Σ_i ||ui||_2^2         
    """
    return param * profileRDD.map(lambda (i, u): np.dot(u, u)).reduce(add)
...
def adaptV(joinedRDD, gamma, mu, N):
    """ Receives as input a joined RDD 
        as well as a gain γ, and regularization parameters λ and μ,  and constructs a new RDD of item profiles of the form 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Parallele Matrix Factorization.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        'data', help='Directory containing folds. The folds should be named fold0, fold1, ..., foldK.')
    parser.add_argument('folds', type=int, help='Number of folds')
    parser.add_argument('--gain', default=0.001, type=float, help="Gain")
    parser.add_argument('--power', default=0.2,
                        type=float, help="Gain Exponent")
    parser.add_argument('--epsilon', default=1.e-99,
                        type=float, help="Desired objective accuracy")
    parser.add_argument('--lam', default=1.0, type=float,
                        help="Regularization parameter for user features")
    parser.add_argument('--mu', default=1.0, type=float,
                        help="Regularization parameter for item features")
    ...
    verbosity_group.add_argument(
        '--silent', dest='verbose', action='store_false')
    ...
    for k in folds:
        ...
        print "Training set contains %d users and %d items" % (
            U.count(), V.count())
        ...
        while i < args.maxiter and change > args.epsilon:
            ...
            oldObjective = obj
            obj = SE(joinedRDD) + normSqRDD(U, args.lam) + \
                normSqRDD(V, args.lam)
            change = np.abs(obj-oldObjective)

            testRMSE = np.sqrt(
                1.*SE(joinAndPredictAll(test, U, V, args.N))/Mtest)
            ...
            now = time()-start
            print "Iteration: %d\tTime: %f\tObjective: %f\tTestRMSE: %f" % (
                i, now, obj, testRMSE)
    ...
    if args.output is None:
        print "%d-fold cross validation error is: %f " % (
            args.folds, np.mean(cross_val_rmses))
    else:
